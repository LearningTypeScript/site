---
date: "July 20 2023"
description: "Why it's not practically feasible -or desirable- for a programming language like TypeScript to use semantic versioning for its releases."
...
:::danger
This is a rough draft of a the blog post and has not been fully proofread, nor approved by the TypeScript team.
Its information might be blatantly incorrect.
Its best practices might actually be the the worst.
Please be cautious and doubtful when reading it, and provide feedback on how it could be better!
:::
...
Unfortunately, only releasing type system changes in major versions is impractical regardless of how often those major versions are released.
...
However, doing so would negate much of the benefits of semver for consumers.
...
But restricting all "breaking" changes to an occasional new major version would mean bottling up all changes for months at a time.
New infrequent major versions would contain a significant amount of type system changes all at once.
Onboarding to new infrequent major versions would be much more difficult than today's minor versions.

Even worse, because the new changes would have so little public user testing in stable versions versions before release, they'd likely contain ma ny more bugs.
...
There are a few strategies you can take to minimize that pain.
...
- `// @ts-expect-error`: Acts like `// @ts-ignore`, but if a type error isn't reported, causes a new type error for the unnecessary comment
...
For more information, see [TypeScript issue #14116](https://github.com/microsoft/TypeScript/issues/14116) that discussed a request to start using semantic versioning: in particular [RyanCavanaugh's second comment](https://github.com/microsoft/TypeScript/issues/14116#issuecomment-292581018).
