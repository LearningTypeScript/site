...
One comment directive sometimes used in TypeScript files is `// @ts-expect-error`, which suppresses type checking for a single line:

```ts
// @ts-expect-error
let value: number = "oops!"; // No type error ✅
```

This might seem counterintuitive: if a project already decided to use TypeScript's type checking, why disable it?

Rare cases do exist when TypeScript's type checking needs to be suppressed in a small area of code.
This post will explain the kinds of comment directives supported by TypeScript: why they exist, what they do, and how to use use them.
...
Suppose you know a global `process` function exists that should take in a `string` parameter, but its types incorrectly describe it as taking in `number`.
...
[`@typescript-eslint/ban-comment`](https://typescript-eslint.io/rules/ban-ts-comment) can be used to enforce best practices with TypeScript's comment directives.
By default, the rule:
...
[`@typescript-eslint/ban-comment`'s `descriptionFormat` option](https://typescript-eslint.io/rules/ban-ts-comment#descriptionformat) can be used to enforce comments align to a certain format.
...
See [typescript-eslint's Getting Started guide](https://typescript-eslint.io/getting-started) to enable `@typescript-eslint/ban-comment` and other recommended rules.
...
[Linters are not the same tools as type checkers](TODO).
