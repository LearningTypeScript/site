...
TypeScript doesn't have a built-in way to describe positive numbers, but we still want to indicate them in the type system.
...
But how do we tell the type system that some new value is known to a branded type?
...
// Ok: the type assertion function enforces that it's a `Positive`. 👍
...
Many thanks to the kind developers who provided feedback on [this article's pull request](https://github.com/LearningTypeScript/site/pull/135):
