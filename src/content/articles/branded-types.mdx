...
async function waitThenLog(seconds: number) {
  ...
  console.log(await waitForSeconds(seconds));

  // This should be an error: -1 is not positive. 👎
  console.log(await waitForSeconds(-1));
...
:::tip
The next version of `ts-brand` will allow `make` to take in a validation function.
