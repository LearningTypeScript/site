...
For our `vibe`, we might have wanted `"happy"` - or maybe even a union of string literals, like as `"happy" | "sad"`.

## Recap: Type Declarations

You can use the `:` type annotation to specify a variable's type.
...
Still, _Learning TypeScript_'s [The Type System chapter](http://localhost:3000/the-type-system) does teach all the concepts you'll need to be able to understand TypeScript's type system.
That incldues how the type system works, the default type inferences mentioned in this article, and explicitly using `:` type annotations.

## Thanks

Many thanks to [John Reilly](https://twitter.com/johnny_reilly) and Kenny Lin for invaluable help in proofreading this article!
